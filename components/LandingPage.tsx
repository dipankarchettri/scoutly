...
export const LandingPage: React.FC<LandingPageProps> = ({ onSearch, onOpenChat }) => {
  ...
  return (
    <div className="min-h-screen bg-transparent text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div
        ref={containerRef}
        className={`z-10 w-full relative flex flex-col items-center transition-all duration-700 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${isExpanded ? 'max-w-3xl' : 'max-w-lg'
          }`}
      >
        ...
        <div className="w-full relative group">
          ...
          <div className={`absolute -inset-[1px] bg-gradient-to-r from-white/10 via-white/5 to-white/10 rounded-2xl blur-[1px] transition-all duration-700 ${isActive ? 'opacity-100 from-lime-500/20 to-emerald-500/20' : 'opacity-70'
            }`}></div>

          <div className={`relative bg-[#050505] rounded-2xl flex items-center shadow-2xl shadow-black/90 p-1.5 ring-1 transition-all duration-500 ${isActive ? 'ring-lime-500/20 bg-black' : 'ring-white/5'
            }`}>
            <form onSubmit={handleSubmit} className="flex-1 flex items-center gap-2 pl-5 pr-1.5">
              ...
              <button
                type="submit"
                className={`h-9 w-9 flex items-center justify-center rounded-xl border transition-all duration-300 ${true // Always enabled
        ...
        <div className={`flex flex-col items-center gap-5 text-center w-full transition-all duration-700 delay-75 ${isExpanded ? 'mt-8 opacity-100' : 'mt-12 opacity-80'
          }`}>
